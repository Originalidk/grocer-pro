...
const getFairPriceProducts = async (search: string, postalCode: string): Promise<Array<ProductInfo> | IError> => {
    try {
        ...
        const promisesList: Array<Promise<Array<FairPriceProduct>>> = []
        ...
        for (let page = 1; page <= totalPages; page++) {
            const productList: Promise<Array<FairPriceProduct>> = fetchPageProductList(page);
            promisesList.push(productList);
        }

        const processedPromises: Array<FairPriceProduct> = (await Promise.all(promisesList)).flat();
...
export default getFairPriceProducts;